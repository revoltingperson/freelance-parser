...
class MainWidget(QWidget, Ui_Form):
    def __init__(self):
        ...
        self.setupUi(self)
        self.setWindowIcon(QIcon(':/favicon-228.ico'))
    ...
    def start_script(self):
        ...
        self.browser_thread.parsed_betting_site.connect(self.process_result_from_thread)
        self.browser_thread.parsed_betting_site.connect(self.thread.quit)
        self.browser_thread.parsed_betting_site.connect(self.browser_thread.deleteLater)

        self.browser_thread.driver_loading.connect(self.notify)
        self.browser_thread.finished.connect(self.enable_start_button)
        self.browser_thread.finished_badly.connect(self.stop_browser_after_exception)
        self.thread.start()

    def stop_browser(self):
        if self.browser_thread.br.running:
            self.stop.setDisabled(True)
            self.browser_thread.br.running = False
            self.thread.quit()
            self.status_label.setText('Останавливаю работу...')

    def stop_browser_after_exception(self):
        self.browser_thread.br.running = False
    ...
    def process_result_from_thread(self, events):
        ...
        for event in events:
            ...
            event_window.setStyleSheet('QFrame#Form {border-radius:10px; border: None; box-shadow: 10px 5px 5px red; background-color: white} QFrame { font-family: Sans-serif }')
...
class BrowserThread(QObject):
    parsed_betting_site = Signal(list)
    br = ChromeLeonBet()
    finished = Signal()
    driver_loading = Signal(str)
    finished_badly = Signal()
    ...
    def run(self):
        ...
        try:
            ...
            while self.br.running:
                status = self.br.run_script()
                if not status:
                    break
                self.parsed_betting_site.emit(self.br.all_games_ready)
                time.sleep(10)
            else:
                self.br.driver.quit()
            self.finished.emit()
            ...
            if self.br.running:
                ...
                self.br.driver.quit()
                self.finished_badly.emit()
                self.driver_loading.emit('Ошибка, проверьте логи, перезапустите бота')
                logger.exception(e)
